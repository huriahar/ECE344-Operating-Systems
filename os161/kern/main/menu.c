...
#endif
	struct thread *most_recent_child;
	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, &most_recent_child);
	if (result) {
...
static
int
cmd_dbflagsmenu(int nargs, char **args)
{
	(void)nargs;
	(void)args;
...
cmd_dbflags(int nargs, char **args){
	...
	if(str_length==1){
		...
		cmd_num = (num[1]-'0') + (num[0]-'0')*10;
	}
	else //unless MAX_DB_CMD is more than 2 digits..
		error = 1;

	char state[128];
	strcpy(state, args[2]);

	if (error || (strcmp(state, "off") != 0 && strcmp(state, "on") !=  0) || cmd_num < 1 || cmd_num  > MAX_DB_CMD){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}
	if (cmd_num == 1) {
